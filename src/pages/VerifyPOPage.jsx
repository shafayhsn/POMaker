...
export default function VerifyPOPage() {
  ...
  const po = useMemo(() => purchaseOrders.find((item) => item.verificationCode === code), [code, purchaseOrders])
      ...
      {!po ? (
        ...
        <div className="card stack-md">
          <div className="section-heading">
            <h2>PO {po.poNo}</h2>
            <Badge tone={po.status === 'Issued' ? 'success' : po.status === 'Draft' ? 'muted' : po.status === 'Cancelled' ? 'danger' : 'warning'}>{po.status}</Badge>
          </div>
          <div className="details-grid">
            ...
            <div><span className="label">Total</span><strong>{formatCurrency(Number(po.total || 0))}</strong></div>
          </div>
        </div>
      )}
    </div>
